...
As an example, a sender may encrypt a message in this way:
        ...
        >>> from CryptoOld.PublicKey import RSA
        >>>
...
import CryptoOld.Signature.PKCS1_PSS
import CryptoOld.Hash.SHA
...
class PKCS1OAEP_Cipher:
    ...
    def __init__(self, key, hashAlgo, mgfunc, label):
        ...
        :Parameters:
         ...
         hashAlgo : hash object
                The hash function to use. This can be a module under `CryptoOld.Hash`
                or an existing hash object created from any of such modules. If not specified,
                `CryptoOld.Hash.SHA` (that is, SHA-1) is used.
         mgfunc : callable
        ...
        if hashAlgo:
            ...
            self._hashObj = CryptoOld.Hash.SHA

        if mgfunc:
            self._mgf = mgfunc
        else:
            self._mgf = lambda x,y: CryptoOld.Signature.PKCS1_PSS.MGF1(x,y,self._hashObj)
    ...
    def encrypt(self, message):
        ...
        modBits = ..Util.number.size(self._key.n)
...
def new(key, hashAlgo=None, mgfunc=None, label=b('')):
    ...
    :Parameters:
     key : RSA key object
      The key to use to encrypt or decrypt the message. This is a `CryptoOld.PublicKey.RSA` object.
