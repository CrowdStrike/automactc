...
out with a complex function (e.g. a CryptoOldgraphic hash) and not by simply
concatenating key and nonce.

New designs should not use ARC4. A good alternative is AES
(`.AES`) in any of the modes that turn it into a stream cipher (OFB, CFB, or CTR).

As an example, encryption can be done as follows:

    >>> from . import ARC4
    >>> from CryptoOld.Hash import SHA
    >>> from . import Random
    >>>
...
def new(key, *args, **kwargs):
    ...
    :Parameters:
      key : byte string
        ...
        Its CryptoOldgrapic strength is always capped to 2048 bits (256 bytes).
