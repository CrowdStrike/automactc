...
CryptoOldgraphic strength of the key (*effective key length*) can be reduced 
via a parameter.

Even though RC2 is not CryptoOldgraphically broken, it has not been analyzed as
    ...
    :Keywords:
      ...
      counter : callable
        ...
        *counter block*, which is a byte string of `block_size` bytes.
        For better performance, use `..Util.Counter`.
      segment_size : integer
        (*Only* `MODE_CFB`).The number of bits the plaintext and ciphertext
        are segmented in.
        It must be a multiple of 8. If 0 or not specified, it will be assumed to be 8.
      effective_keylen : integer
        Maximum CryptoOldgraphic strength of the key, in bits.
